...
function um_optimize_photos__get_meta_sql( $sql, $queries, $type, $primary_table, $primary_id_column, $context ) {
	if ( UM()->options()->get( 'um_optimize_photos' )
		...
		// TESTING.
//		error_log( 'Test photos. JOIN:     ' . $sql['join'] );
//		error_log( 'Test photos. JOIN_UM:  ' . $sql_um['join'] );
//		error_log( 'Test photos. WHERE:    ' . $sql['where'] );
//		error_log( 'Test photos. WHERE_UM: ' . $sql_um['where'] );
